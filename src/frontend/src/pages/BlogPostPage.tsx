...
import { useAllBlogPosts, useBlogPostBySlug } from "@/hooks/useBackendQuery";
...
const FALLBACK_CONTENT = [
  ...
  '> "The mountains teach patience. Every trail, every weather window, every step above 4,000 metres is a lesson in accepting what you cannot control." — Manya Destination Team',
  "## The Journey Begins",
...
const AUTHOR_BIOS: Record<string, string> = {
  "Arjun Mehta":
    "Senior trek photographer and expedition leader with over 200 Himalayan days. Arjun has contributed to National Geographic India and leads Manya Destination's high-altitude photography workshops.",
  "Priya Sharma":
  ...
  "Vikram Nair":
    "Lead trek guide and author of the Manya Destination field manuals. Vikram has summited Kedarkantha, Roopkund, and Brahmatal collectively over forty times.",
  "Sunita Rawat":
    "Born in Uttarkashi, Sunita brings deep insider knowledge of Garhwal's pilgrimage routes. She is Manya Destination's resident Char Dham Yatra advisor.",
};
...
export default function BlogPostPage() {
  ...
  const { data: backendPost, isLoading } = useBlogPostBySlug(slug);
  const { data: allBackendPosts = [] } = useAllBlogPosts();
  ...
  const authorBio = post
    ? (AUTHOR_BIOS[post.authorName] ??
      `${post.authorName} is a travel writer and guide at Manya Destination.`)
    : "";
    ...
    <div className="min-h-screen bg-background">
      ...
      {relatedPosts.length > 0 && (
        <section
          ...
          style={{ background: "var(--bg-secondary, #f5f0e8)" }}
        >
